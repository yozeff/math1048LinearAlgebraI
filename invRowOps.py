...
def inv(user_mat):
	...
	#we can compute the determinant to check it's non-zero and bring the augented matrix to row echelon
	#form
	if detRowOps.det(mat) == 0:
		return None
	else:
		#now we can bring the augmented matrix to a reduced row echelon form and read off the inverse
...
def main():
	...
	if inv_mat == None:
		print("determinant is 0 so inverse doesn't exist")
	else:
		print('\ninverse matrix (5d.p):\n')
		res.print_mat(inv_mat, 5)
